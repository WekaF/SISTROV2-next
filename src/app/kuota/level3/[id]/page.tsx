...
function KuotaLevel3Content() {
  ...
  const [data, setData] = useState<Level3Row[]>([])
  const [error, setError] = useState<string | null>(null)

  const [editModal, setEditModal] = useState<{ open: boolean; row: Level3Row | null }>({ open: false, row: null })
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true)
      try {
        const res = await fetch(`/api/kuota/level3/${id}`)
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-3">
        ...
        <p className="text-gray-500 animate-pulse">Memuat data area...</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <nav className="flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400">
        ...
        {header && (
          <>
            <button
              onClick={() => l1 ? router.push(`/kuota/level2/${l1}`) : router.back()}
              className="hover:text-brand-500 transition-colors"
            >
              Level 2 {header.wilayahNama ? `(${header.wilayahNama})` : ""}
            </button>
            <ChevronRight className="h-3.5 w-3.5" />
          </>
        )}
        <span className="text-gray-900 dark:text-white font-semibold">Detail Area</span>
      </nav>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white font-serif">
            Kuota Level 3 — Per Area
          </h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Distribusi kuota ke setiap area / bagian dalam wilayah.
          </p>
        </div>
        ...
        <Card className="shadow-theme-xs">
          <CardContent className="pt-5">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div>
                <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-1">Wilayah</p>
                <div className="flex items-center gap-1.5 font-semibold text-gray-900 dark:text-white">
              ...
              <div>
                <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-1">Kuota Wilayah</p>
                <p className="font-bold text-xl text-brand-600">
                  {Number(header.kuotaWilayah).toLocaleString("id-ID")}
      ...
      <Card className="shadow-theme-xs">
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-gray-900 dark:text-white">Distribusi per Area</span>
            <span className="ml-auto text-xs text-gray-400">{data.length} area</span>
          </div>
        </CardHeader>
        <CardContent>
            ...
            <table className="w-full text-left min-w-[1100px]">
              <thead className="bg-gray-50 dark:bg-white/[0.02]">
                <tr className="border-b border-gray-100 dark:border-gray-800">
                  ...
                  <th className="px-4 py-3 text-xs font-bold uppercase text-gray-400 tracking-wider">Area / Bagian</th>
                  <th className="px-4 py-3 text-xs font-bold uppercase text-gray-400 tracking-wider">Wilayah</th>
                  <th className="px-4 py-3 text-xs font-bold uppercase text-gray-400 tracking-wider">Produk</th>
                  <th className="px-4 py-3 text-xs font-bold uppercase text-gray-400 tracking-wider text-right">Kuota Wilayah</th>
                  ...
                  data.map((item, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="font-semibold text-gray-900 dark:text-white text-sm">{item.bagianNama}</div>
                        <div className="text-xs text-gray-400">{item.bagianAbbrev}</div>
                      </td>
                      ...
                      <td className="px-4 py-3 text-right font-bold text-gray-500 text-sm">
                        {Number(header?.kuotaWilayah || 0).toLocaleString("id-ID")}
                      </td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                          <Button
                            ...
                            onClick={() => router.push(`/kuota/level4/${item.guid}?l2=${id}&l1=${l1}`)}
                          >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/50 backdrop-blur-sm px-4">
          <div className="bg-white dark:bg-gray-950 rounded-2xl w-full max-w-md shadow-2xl border border-gray-100 dark:border-gray-800">
            <div className="p-6 border-b border-gray-100 dark:border-gray-800">
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">Edit Kuota Area</h2>
              <p className="text-sm text-gray-500 mt-1">
                {editModal.row.bagianNama} ({editModal.row.bagianAbbrev})
              </p>
            </div>
...
export default function KuotaLevel3Page() {
  return (
    <Suspense fallback={
      <div className="flex flex-col items-center justify-center py-24 gap-3">
        <div className="w-10 h-10 border-4 border-brand-500 border-t-transparent rounded-full animate-spin" />
