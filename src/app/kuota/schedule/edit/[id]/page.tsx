...
export default function EditQuotaWizard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Fetch Quota Details for Editing
        if (id) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (data.success) {
        addToast({ variant: "success", title: "Kuota berhasil diperbarui" });
      ...
      <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 pb-6 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
            ...
            Edit Penjadwalan Kuota
          </h1>
          <p className="text-sm text-gray-500">Perbarui alokasi tonase distribusi per shift secara sistematis.</p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`p-2 rounded-xl ${isActive ? "bg-brand-500 text-white" : isCompleted ? "bg-emerald-500 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-400"
                    }`}>
                ...
                {lookup.areas.filter(a => formData.areas[a.id] > 0).map(a => (
                  <div key={a.id} className={`p-5 border rounded-2xl ${(() => {
                    ...
                    <div
                      ...
                      style={{ "--shift-cols": (Number(a.tipe) > 0 ? Number(a.tipe) : lookup.shiftCount) } as React.CSSProperties}
                    >
                      {Array.from({ length: (Number(a.tipe) > 0 ? Number(a.tipe) : lookup.shiftCount) }, (_, i) => i + 1).map(sNum => (
