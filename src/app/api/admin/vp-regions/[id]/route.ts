...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const res = await aspnetFetchServer("/api/VpRegion/Rename", token, {
      method: "POST",
    ...
    return NextResponse.json({ success: true, data: toRegion(region) });
  } catch (error: any) {
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await params;
    const token = (session?.user as any)?.aspnetToken as string;
    const res = await aspnetFetchServer("/api/VpRegion/Delete", token, {
      method: "POST",
