/**
 * POST /api/tiket/detail-data
 * API Next.js yang langsung query SQL Server (SISTROSTAGING)
 * sebagai pengganti ASP.NET /api/Tiket/DetailData yang bermasalah.
 *
  ...
  try {
    ...
    queryStr += `
        WHERE (t.bookingno = @bookingno OR t.tiketno = @bookingno)
          AND (p.company_code = @companyCode OR p.company_code IS NULL OR @companyCode = '')
    `;

    // ── 1. Query tiket utama dengan join ke Posto, Kuota4Shift, Transport, Produk ──
    ...
    if (!tiketRes.recordset || tiketRes.recordset.length === 0) {
      return NextResponse.json(
        { error: "Nomor tiket tidak dikenali atau Company tidak sesuai" },
        { status: 400 }
      );
    }

    const row = tiketRes.recordset[0];
    const co = row.company ?? companyCode;
