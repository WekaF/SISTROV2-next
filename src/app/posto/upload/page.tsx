...
export default function PostoUploadPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        if (resData.records || resData.data?.records) {
          setValidationData(resData.data || resData);
        } else {
          setSuccess(true);
          setSuccessMsg(resData.message || "Upload POSTO berhasil!");
        }
      } else {
        alert("Gagal proses file: " + (resData.message || resData.error || res.statusText));
      }
    } catch (error) {
      console.error(error);
  ...
  const handleSubmit = async () => {
    ...
    setUploading(true);
    try {
      const res = await apiFetch("/api/POSTO/SubmitData", {
        method: "POST",
        body: JSON.stringify({
          records: validationData.records.filter((r: any) => !r.isDuplicate),
          wilayah: selectedWilayah?.abbrev,
          tipe: validationData.context?.tipe
        })
      });
      ...
      if (res.ok) {
        setSuccess(true);
        setSuccessMsg(resData.message || "Data POSTO berhasil disimpan!");
        setValidationData(null);
      } else {
        alert("Gagal menyimpan data: " + (resData.message || resData.error));
      }
    } catch (error) {
      console.error(error);
            ...
            <div className="p-4 bg-blue-50 border border-blue-100 dark:bg-blue-500/10 dark:border-blue-500/20 rounded-xl flex items-start gap-3">
              ...
              <div className="text-sm text-blue-700 dark:text-blue-400">
                ...
                <ul className="list-disc list-inside space-y-1 text-xs">
                  ...
                  <li>Gunakan kolom <span className="font-bold">idsumbu</span> untuk membedakan jenis sumbu kendaraan.</li>
