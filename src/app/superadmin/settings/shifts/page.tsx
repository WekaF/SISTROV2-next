...
import {
  CalendarClock, Edit, X, Loader2, Clock, ShieldCheck, 
  Plus, Timer, Activity, Save
} from "lucide-react";
...
export default function ShiftManagementPage() {
  ...
  const stats = {
    total: Array.isArray(allShifts) ? allShifts.length : 0,
    scopes: Array.isArray(allShifts) ? new Set(allShifts.map(s => s.scope)).size : 0,
  };
  ...
  const columns: DataTableColumn<any>[] = [
    ...
    },
    {
      ...
      className: "text-center",
      render: (item) => (
        <span className="font-mono text-xs font-bold">{item.level}</span>
      ),
    },
    {
      key: "starttime",
  ...
  const handleEdit = async (item: any) => {
    try {
      const res = await fetch(`/api/admin/shifts?abbrev=${item.abbrev}`, {
        method: 'PATCH'
      });
      const detail = await res.json();
      
      if (res.ok) {
        // Format time strings if they are full dates
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-gray-400 max-w-2xl font-medium leading-relaxed uppercase text-[10px] tracking-[0.2em]">
              Pengaturan pembagian waktu operasional dan shift kerja di seluruh area operasional sistro.
            </p>
          </div>
