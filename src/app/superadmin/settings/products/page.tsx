...
export default function ProductMasterPage() {
  ...
  // Query for companies (mapped to getCompanyListFitur)
  ...
  const handleEditProduct = (e: React.FormEvent) => {
    e.preventDefault();
    updateProductMutation.mutate(editFormData as Product);
  };

  const addMappingMutation = useMutation({
    mutationFn: async ({ productId, companyCodes }: { productId: number, companyCodes: string[] }) => {
    ...
    onSuccess: (_, variables) => {
      addToast({ title: "Mapping Berhasil", description: `${variables.companyCodes.length} plant telah dipetakan ke produk.`, variant: "success" });
  ...
  const handleAddMapping = () => {
    if (!selectedProduct || selectedCompanies.length === 0) return;
    addMappingMutation.mutate({ productId: selectedProduct.id, companyCodes: selectedCompanies });
  };
            ...
            <table className="w-full text-left">
              <thead className="bg-gray-50 dark:bg-white/[0.01]">
                <tr className="border-b border-gray-100 dark:border-gray-800">
                  <th className="px-6 py-4 text-[10px] font-black uppercase text-gray-500 tracking-widest">ID Produk</th>
                  <th className="px-6 py-4 text-[10px] font-black uppercase text-gray-500 tracking-widest">Nama Produk</th>
                  <th className="px-6 py-4 text-[10px] font-black uppercase text-gray-500 tracking-widest">Status Produk</th>
          ...
          <Card className="w-full max-w-md shadow-2xl border-none bg-white dark:bg-[#1a1c1e]">
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>Ubah detail informasi produk.</CardDescription>
              </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    id="editIsSubsidi"
                    className="rounded border-gray-300 text-brand-500 focus:ring-brand-500"
                    checked={editFormData.isSubsidi || false}
                    onChange={(e) => setEditFormData({ ...editFormData, isSubsidi: e.target.checked })}
                  />
                  <label htmlFor="editIsSubsidi" className="text-sm font-medium text-gray-700 dark:text-gray-300">Produk Subsidi</label>
                </div>
              </CardContent>
              <CardFooter className="flex justify-end gap-2 border-t border-gray-100 dark:border-gray-800 pt-6">
                <Button variant="ghost" type="button" onClick={() => setShowEditModal(false)}>Batal</Button>
                <Button type="submit" className="bg-brand-500 hover:bg-brand-600" disabled={updateProductMutation.isPending}>
