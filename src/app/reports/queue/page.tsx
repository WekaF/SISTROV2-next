...
export default function LaporanAntrianPage() {
  ...
  const handleExportExcel = async () => {
    ...
    setExporting(true);
    const headers = [
      "No Tiket", "Nopol", "Driver", "Booking No", "Produk", "POSTO", "Asal", "Tujuan", "Gudang Muat", "Qty (ton)", "Status Muat", "Posisi", "Updated On"
    ];
    const keys = [
      "tiketno", "nopol", "driver", "bookingno", "produkString", "posto", "asal", "tujuan", "gudangMuat", "qty", "statuspemuatan", "positionString", "updatedonString"
    ];
    const { exportToExcel } = await import("@/lib/export-helper");
    exportToExcel(data, headers, keys, `Laporan_Antrian_${new Date().toISOString().slice(0, 10)}`);
    setExporting(false);
  };
  ...
  const handleExportPdf = async () => {
    ...
    exportToPdf(data, headers, keys, `Laporan Antrian Aktif (${new Date().toLocaleDateString("id-ID")})`);
    setExporting(false);
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 border border-[#e2e8f0] shadow-sm gap-4">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <Button
            ...
            className="rounded-none border-emerald-600 text-emerald-600 hover:bg-emerald-50 dark:border-emerald-500 dark:text-emerald-500 dark:hover:bg-emerald-950 font-bold h-12 px-4"
          >
            EXCEL
          </Button>
          <Button
            ...
            disabled={exporting || isLoading}
            className="rounded-none border-rose-600 text-rose-600 hover:bg-rose-50 dark:border-rose-500 dark:text-rose-500 dark:hover:bg-rose-950 font-bold h-12 px-4"
          >
            PDF
          </Button>
        </div>
        ...
        <div className="flex gap-6 h-full min-w-max pr-6">
          {isLoading && !data ? (
            ...
            Object.entries(groupedData).map(([warehouse, trucks]) => (
              ...
              >
                <div className="bg-[#0f172a] p-5 border-b-4 border-[#10b981]">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="bg-[#10b981] text-white font-black text-sm px-2.5 py-0.5 shrink-0">
                      {trucks.length}
                    </div>
                  </div>
                  <div className="flex items-center mt-2">
                    <MapPin className="w-3 h-3 text-[#10b981] mr-1.5" />
                    <span className="text-[10px] font-bold text-[#94a3b8] uppercase tracking-[0.2em]">Gudang Antrian</span>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#f8fafc]">
                  {trucks.map((truck, idx) => (
                    <div
                      key={truck.id || idx}
                      className="bg-white border-l-4 border-[#10b981] border-y border-r border-[#e2e8f0] shadow-sm hover:shadow-md transition-all"
                    >
                      <div className="p-4">
                        <div className="flex justify-between items-start mb-4">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <div className="text-[10px] font-black text-[#10b981] uppercase tracking-widest">
                                {truck.driver || "UNASSIGNED DRIVER"}
                              </div>
                            </div>
                          </div>
                          <div className="text-[10px] font-black text-[#94a3b8] opacity-30">#{truck.number}</div>
                        </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex justify-between items-center bg-[#1e293b] p-2 px-3">
                            ...
                            <div className="text-xs font-black text-white">
                              {truck.qty} <span className="text-[#10b981]">TON</span>
                            </div>
                          </div>
                        </div>
                      </div>
